  ...
  return (
    <div className="min-h-screen w-full  bg-white ">
        <div className="w-full max-w-7xl  mx-auto px-1 py-10 sm:px-5 sm:py-15 space-y-8">
            <h2 className="font-semibold text-6xl md:text-7xl overflow-hidden py-2 capitalize ">Brands</h2>
            ...
            <div className='flex items-center justify-center gap-1 sm:gap-2'>
                {
                    categories.map((item ,index) => (
                        ...
                        <button key={index} onClick={() => setActiveBrand(item)} className={activeBrand === item ? ' p-2 sm:px-4 py-2 rounded-full text-xs sm:text-sm bg-black text-white font-semibold' : ' p-2 sm:px-4 py-2 rounded-full text-xs sm:text-sm bg-gray-200/40'}>
                            {item}
                        </button>
                    ))
                }
